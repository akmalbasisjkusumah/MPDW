...
```{r}
data <- read.csv("https://raw.githubusercontent.com/akmalbasisjkusumah/MPDW/main/Pertemuan%202/Yearly-test.csv")
data
```
...
ts.plot(data.ts, xlab="Time Period ", ylab="V5", main= "Time Series Plot")
points(data.ts)
```
...
plot(training.ts, col="blue",main="Plot training")
points(training.ts)
```
