...
```{r}
library(rio)
data <- import("https://raw.githubusercontent.com/akmalbasisjkusumah/MPDW/main/Pertemuan%201/Dataset%20Tugas%201.csv")
data
```
...
#Menyimpan Plot
dev.copy(png, "Plot Deret Waktu.png")
dev.off()
```
...
data.gab #forecast 25 periode ke depan
```

Adapun plot data deret waktu dari hasil peramalan yang dilakukan adalah sebagai berikut.

```{r}
ts.plot(data.ts, xlab="Periode Waktu", ylab="Harga Beras", main= "SMA N=4 Data Harga Beras")
points(data.ts)
...
Dalam hal ini nilai MAPE data latih pada metode pemulusan SMA kurang dari 3%, nilai ini dapat dikategorikan sebagai nilai akurasi yang sangat baik. Selanjutnya dilakukan perhitungan nilai MAPE data uji pada metde pemulusan SMA.
...
```{r}
ts.plot(data.ts, xlab="Periode Waktu", ylab="Harga Beras", main= "DMA N=4 Data Harga Beras")
points(data.ts)
...
training<-data[1:103,]
testing<-data[104:128,]
train.ts <- ts(training$`Harga Beras`)
test.ts <- ts(testing$`Harga Beras`)
```
...
#ramalan
ramalanopt<- forecast(sesopt, h=25)
ramalanopt
```
...
resid1<-training$`Harga Beras`-ramalan1$fitted
head(resid1)
```
...
resid2<-training$`Harga Beras`-ramalan2$fitted
head(resid2)
...
```{r}
selisih1<-ramalan1$mean-testing$`Harga Beras`
SSEtesting1<-sum(selisih1^2)
MSEtesting1<-SSEtesting1/length(testing)

selisih2<-ramalan2$mean-testing$`Harga Beras`
SSEtesting2<-sum(selisih2^2)
MSEtesting2<-SSEtesting2/length(testing)

selisihopt<-ramalanopt$mean-testing$`Harga Beras`
...
```{r}
#cara lain
accuracy(ramalanopt,testing$`Harga Beras`)
```
...
selisihdes1<-ramalandes1$mean-testing$`Harga Beras`
selisihdes1

SSEtestingdes1<-sum(selisihdes1^2)
MSEtestingdes1<-SSEtestingdes1/length(testing$`Harga Beras`)
MAPEtestingdes1<-sum(abs(selisihdes1/testing$`Harga Beras`)*100)/length(testing$`Harga Beras`)

selisihdes2<-ramalandes2$mean-testing$`Harga Beras`
selisihdes2

SSEtestingdes2<-sum(selisihdes2^2)
MSEtestingdes2<-SSEtestingdes2/length(testing$`Harga Beras`)
MAPEtestingdes2<-sum(abs(selisihdes2/testing$`Harga Beras`)*100)/length(testing$`Harga Beras`)

selisihdesopt<-ramalandesopt$mean-testing$`Harga Beras`
selisihdesopt

SSEtestingdesopt<-sum(selisihdesopt^2)
MSEtestingdesopt<-SSEtestingdesopt/length(testing$`Harga Beras`)
MAPEtestingdesopt<-sum(abs(selisihdesopt/testing$`Harga Beras`)*100)/length(testing$`Harga Beras`)
...
#Import data
library(rio)
data3 <- import("https://raw.githubusercontent.com/akmalbasisjkusumah/MPDW/main/Pertemuan%201/Dataset%20Tugas%201.csv")
...
selisih1<-forecast1-testing.ts[1:25,]
SSEtesting1<-sum(selisih1^2)
MSEtesting1<-SSEtesting1/length(testing.ts)

forecast1.opt<-data.frame(forecast1.opt)
selisih1.opt<-forecast1.opt-testing.ts[1:25,]
...
testing.ts<-data.frame(testing.ts)
selisih2<-forecast2-testing.ts[1:25,]
SSEtesting2<-sum(selisih2^2)
MSEtesting2<-SSEtesting2/length(testing.ts)

forecast2.opt<-data.frame(forecast2.opt)
selisih2.opt<-forecast2.opt-testing.ts[1:25,]
